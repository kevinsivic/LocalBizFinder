...
const customServerUrl = "https://api.track.jemsoftware.co/log";
...
const initializeAnalytics = async () => {
  try {
    ...
    if (!projectId) {
      console.debug(
        "[Analytics] No project ID in client environment, fetching from server...",
      );
      const response = await fetch("/api/config");
      const config = await response.json();
      projectId = config.SWETRIX_PROJECT_ID;
      console.debug(
        "[Analytics] Fetched project ID from server:",
        projectId || "(not set)",
      );
    }
    ...
    if (shouldInitialize) {
      ...
      console.debug(
        `[Analytics] Initialized with project ID: ${projectId}, custom server: ${customServerUrl}`,
      );
      Swetrix.trackViews();
    } else {
      console.warn(
        "[Analytics] Skipping initialization - no project ID available",
      );
    }
  } catch (error) {
